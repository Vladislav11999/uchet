...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if now - last_time < 5:
        logging.info(f"Антифлуд: пользователь {user_id} слишком быстро вызвал /start")
        return

    user_last_command[user_id] = now
    logging.info(f"Команда /start от пользователя: {user_id} ({update.effective_user.username})")
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    logging.info(f"Нажата кнопка: {query.data} от {query.from_user.id} ({query.from_user.username})")
